...
def run():
    ...
    with open("covid_19_data.csv") as file:
        ...
        for line in csv_reader:
            newrecord = line
            covid_records.append(newrecord)
    print(totalnumber(covid_records))
    ...
    while True:
        ...
        if x == 1:
            ...
            x = menu(x)
            if x == 11:
                progress("retrieval process", 0)
                display_record(retrieve_record(covid_records))
                progress("retrieval process", 100)
                progress("data processing", 100)
            if x == 12:
                progress("records retrieval process", 0)
                display_records(retrieve_observ_dates(covid_records))
                progress("records retrieval process", 100)
                progress("data processing", 100)
            if x == 13:
                progress("grouping process", 0)
                display_records(grouped_info(covid_records,input("give country: ")))
                progress("grouping process", 100)
                progress("data processing", 100)
            if x == 14:
                progress("summary process",0)
                summary_print(summary(covid_records))
        ...
        # data visualisation operation has completed.
            ...
            x = menu(x)
            progress("visualisation operation", 0)
            if x == 21:
                display_pie(summary(covid_records))
                progress("visualisation operation", 100)
            if x == 22:
                bar_chart(summary(covid_records))
                progress("visualisation operation", 100)
        ...
        # You should use these to write the records (either all or only those for a specific country/region) to a JSON file.
            ...
            x = menu(x)
            if x == 31:
                progress("exporting operation", 0)
                progress("exporting operation", 100)
            if x == 32:
                progress("partial exporting operation", 0)
                progress("partial exporting operation", 100)
